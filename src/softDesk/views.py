...
class IssuesViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if serializer.is_valid(raise_exception=True):
            Issues.objects.create(title=issue_data['title'], description=issue_data['description'], tag=issue_data['tag'], project_id=project_id, priority=issue_data['priority'], status=issue_data['status'], author_user=user, assignee_user=assignee_user_id)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class CommentsViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if serializer.is_valid():
            Comments.objects.create(description=comment_data['description'], author_user=user, issue_id=comment_data['issue'])
            return Response(serializer.data, status=status.HTTP_201_CREATED)
